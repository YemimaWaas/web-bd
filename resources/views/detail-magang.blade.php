...
<body>
    ...
    <main>
        <div class="container">
            <div class="card mb-3">
                <img src="{{ asset('storage/artikel/' . $artikel->image) }}" class="card-img-top mx-auto d-block alt="{{ $artikel->judul }}" style="max-width: 200px; max-height: 200px">
                <div class="card-body">
                    <p class="mb-3 text-secondary">{{ $artikel->create_at}}</p>
                    <h5 class="card-title">{{ $artikel->judul }}</h5>
                    <p class="card-text">{!! $artikel->desc !!}</p>

                    {{-- Tambahkan bagian formulir --}}
                    @if ($artikel->formulir)
                    <p><a href="{{ asset('storage/formulir/' . $artikel->formulir) }}" download>Unduh Formulir</a></p>
                    @endif
                </div>
            </div>
        </div>
